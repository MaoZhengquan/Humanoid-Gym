...
class GR1WalkPhase(Humanoid):
    ...
    def friction_0_6(self):
        flag_0 = (self.dof_vel[:, 0] <= 0.002) & (self.dof_vel[:, 0] >= -0.002) # 静止或者接近静止
        flag_1 = ((self.dof_vel[:, 0] > 0.002) & (self.dof_vel[:, 0] <= 0.16))  # 低速向前移动
        flag_2 = (self.dof_vel[:, 0] > 0.16)                                    # 高速向前移动
        flag_3 = ((self.dof_vel[:, 0] < -0.002) & (self.dof_vel[:, 0] >= -0.16))# 低速向后移动
        flag_4 = (self.dof_vel[:, 0] < -0.16)                                   # 高速向后移动
