...
class DemoNohlaRLWalk:
    ...
    def __init__(
        ...
        self.sess = ort.InferenceSession('/home/gr124ja0052/Fourier/fourier-grx-client/examples/data/1020/policy.onnx',
                                         session_options)
        # warm up
        time.sleep(5)
    ...
    def set_gains(self):
        ...
        # fmt: off
        pd_kp = [
            251.625, 362.52, 200, 200, 10.98, 0,  # left leg
            251.625, 362.52, 200, 200, 10.98, 0,  # right leg
    ...
    def control_thread(self):
        while True:
            ...
            if duration_time > 0.002:
                # print("control timeout")
                continue
            else:
                time.sleep(0.002 - duration_time)
    ...
    def sensor_data_thread(self):
        while True:
            global hist_obs, joint_target_position_deg, joint_target_position_deg_fake,data
            start_time = time.time()
            ...
            data = pd.read_csv('/home/gr124ja0052/Fourier/fourier-grx-client/examples/obs/obs_mujoco.csv', skiprows=92,
                               header=None)
            mujoco_ang_left = data.iloc[:, 34:39].values * (180.0 / math.pi)
            mujoco_ang_right = data.iloc[:, 39:44].values * (180.0 / math.pi)
            print("len(mujoco_ang)", len(mujoco_ang_left))
            if self.count_lowlevel < len(mujoco_ang_left) + 50:
                index = int((self.count_lowlevel - 50))
                print("index", index)
                joint_target_position_deg = self.joint_default_position * 180.0 / math.pi
                # joint_target_position_deg[0:5] += mujoco_ang_left[index] * 0.5
                joint_target_position_deg[2] += mujoco_ang_left[index,2] * 0.5
                joint_target_position_deg[8] += mujoco_ang_left[index,2] * 0.5
                # joint_target_position_deg[6:11] += mujoco_ang_right[index] * 0.5

            else:
                # 一旦到达最后一个数据，持续传递最后一个数据
                joint_target_position_deg = self.joint_default_position * 180.0 / math.pi
                # joint_target_position_deg[0:5] += mujoco_ang_left[-1] * 0.5
                joint_target_position_deg[2] += mujoco_ang_left[-1,2] * 0.5
                joint_target_position_deg[8] += mujoco_ang_left[-1,2] * 0.5
                # joint_target_position_deg[6:11] += mujoco_ang_right[-1] * 0.5
                print("over", self.count_lowlevel)
            obs_all = np.zeros(56, dtype=np.float32)
            obs_all[:44] = obs
            obs_all[44:56] = joint_target_position_deg[:12] / 180.0 * math.pi
