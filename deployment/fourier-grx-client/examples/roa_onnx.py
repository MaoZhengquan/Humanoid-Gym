...
class DemoNohlaRLWalk:
    ...
    def __init__(
        ...
        self.sess = ort.InferenceSession('/home/gr124ja0052/Fourier/fourier-grx-client/examples/data/1023/policy.onnx',
                                         session_options)
        # warm up
        time.sleep(5)
    ...
    def set_gains(self):
        ...
        """
        pos_kp = [
            0.997, 1.023, 1.561, 1.561, 0.508, 0.508,
            0.997, 1.023, 1.561, 1.561, 0.508, 0.508,
        ...
        vel_kp = [
            0.044, 0.03, 0.52, 0.52, 0.004, 0.004,
            0.044, 0.03, 0.52, 0.52, 0.004, 0.004,
            0.03, 0.03, 0.03,
            0.03, 0.03, 0.03,
        ...
        self.client.set_gains(position_control_kp=pos_kp,velocity_control_kp=vel_kp,pd_control_kp=pd_kp, pd_control_kd=pd_kd)
        ...
        while True:
            try:
                ...
                if self.count_lowlevel == 0:
                    joint_target_position_deg_fake = np.zeros(32)
                    joint_target_position_deg_fake = np.rad2deg(self.joint_default_position)
                    joint_target_queue.put(joint_target_position_deg_fake)
                start_time = time.time()
                # 从队列中获取关节角度数据
                if not joint_target_queue.empty():
                    # print('1')
                    joint_target_position_deg = joint_target_queue.get_nowait()
                    self.last_target_pos = joint_target_position_deg
                else:
                    # print('empty')
                    joint_target_position_deg = self.last_target_pos
                ...
                # print(self.count_lowlevel)
            except queue.Empty:
                pass
            end_time = time.time()
            duration_time = end_time - start_time
            # print("control_time",duration_time)
            # print("control_timestep", time.time() - self.start_time)
            if duration_time > 0.001:
                # print("control timeout")
                continue
            else:
                time.sleep(0.001 - duration_time)

    def compute_pd_tau(self,target_q,q,dq):
        pd_kp = np.array([
            200, 200, 300, 300, 10.98, 0,  # left leg
            200, 200, 300, 300, 10.98, 0,  # right leg
        ])
        pd_kd = np.array([
            20, 20, 30, 30, 0.6, 0.1,  # left leg
            20, 20, 30, 30, 0.6, 0.1,  # right leg
        ])
    ...
    def sensor_data_thread(self):
        while True:
            global hist_obs, joint_target_position_deg, joint_target_position_deg_fake
            start_time = time.time()
            ...
            commands[0] = -0.2 * self.cfg.obs_scale.lin_vel
            ...
            right_leg_phase = math.sin(2 * math.pi * self.count_lowlevel * self.cfg.dt / self.cfg.cycle_time)
            left_leg_phase = math.cos(2 * math.pi * self.count_lowlevel * self.cfg.dt / self.cfg.cycle_time)
            ...
            joint_target_pos_sin = np.zeros(32)
            joint_target_pos_sin += self.joint_default_position
            # print(self.count_lowlevel // 30 % 2)
            if (self.count_lowlevel // 30 % 2) == 0:
                print(0)
                if self.count_lowlevel % 30 <= 10:
                    joint_target_pos_sin[2] += 0.11 / 10.0 * (self.count_lowlevel % 30)
                elif self.count_lowlevel % 30 <= 20:
                    joint_target_pos_sin[2] += 0.11
                else:
                    joint_target_pos_sin[2] += -0.11 / 10.0 * (self.count_lowlevel % 30) + 0.33
            else:
                print(1)
                if self.count_lowlevel % 30 <=10:
                    joint_target_pos_sin[2] += 0.22 /10.0 * (self.count_lowlevel % 30)
                elif self.count_lowlevel % 30 <=20:
                    joint_target_pos_sin[2] += 0.22
                else:
                    joint_target_pos_sin[2] += -0.22 / 10.0 * (self.count_lowlevel % 30) +0.66
            # joint_target_pos_sin[2] += math.sin(2 * math.pi * self.count_lowlevel * self.cfg.dt / 0.4) * 0.22
            joint_target_pos_sin = np.rad2deg(joint_target_pos_sin)
            joint_target_queue.queue.clear()
            # print("队列是否为空",joint_target_queue.empty())
            joint_target_queue.put(joint_target_pos_sin)
            # joint_target_queue.put(joint_target_position_deg)
            obs_all = np.zeros(56, dtype=np.float32)
            obs_all[:44] = obs
            obs_all[44:56] = joint_target_pos_sin[:12] / 180.0 * math.pi
            if self.count_lowlevel > 50:
                record_to_csv(obs_all.reshape(1,56),'/home/gr124ja0052/Fourier/fourier-grx-client/examples/obs/obs.csv')
...
def main(
    ...
    thread_control = threading.Thread(target=walker.control_thread)
